...
- **备注**:关(默认)时对表**零操作**(不调 DescribeTable/CreateTable),保持「运维自行建表」的老行为。开启需额外 IAM 权限 `dynamodb:DescribeTable` +(建表用)`dynamodb:CreateTable`。**兼容性检查会拦住把 v1(String 键)表与 v2(Binary 键)表用错线的经典坑**——例如 v2 代理指向 v1 的 S 键表会 fatal 并提示「这看起来是 v1(String 键)表,请用 v1 代理」,反之亦然。

...
for n, ar, s, fk in rows:
    ...
    if n in via:
        ...
        disp, need, reason = 'unsupported', '否', 'Lua 脚本：超范围'
    elif n in txn:
        disp, need, reason = 'unsupported', '否', '事务：超范围（无原子多命令）'
    elif n in bit:
        disp, need, reason = 'unsupported', '否', '位运算：超范围'
    elif n in hll:
        disp, need, reason = 'unsupported', '否', 'HyperLogLog：超范围'
    elif n in geo:
        disp, need, reason = 'unsupported', '否', 'GEO：超范围'
    elif n in block:
        disp, need, reason = 'unsupported', '否', '阻塞命令：需长连接阻塞语义'
    elif n in flush:
        disp, need, reason = 'unsupported', '否', '全表清空：未支持'
    elif n in keymgmt:
        disp, need, reason = 'unsupported', '否', '键管理：DynamoDB 表达不了或超范围'
    elif n in notimpl:
        disp, need, reason = 'unsupported', '否', '★可经 redimo 实现，尚未注册'
    elif n in admin:
        disp, need, reason = 'unsupported', '否', '服务器/复制/管理：超范围'
    else:

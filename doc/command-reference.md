...
| 命令 | sflags | firstkey | 键空间 | 家族 | 走 redimo | 原因 |
|---|---|---:|:---:|---|:---:|---|
| `asking` | `F` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `bgrewriteaof` | `a` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `bgsave` | `a` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `blpop` | `ws` | 1 | 是 | — | 否 | 阻塞命令：架构受限（需长连接阻塞语义） |
| `brpop` | `ws` | 1 | 是 | — | 否 | 阻塞命令：架构受限（需长连接阻塞语义） |
| `brpoplpush` | `wms` | 1 | 是 | — | 否 | 阻塞命令：架构受限（需长连接阻塞语义） |
| `cluster` | `a` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `debug` | `as` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `discard` | `sF` | 0 | 否 | — | 否 | 事务：架构受限（需原子多命令） |
| `dump` | `r` | 1 | 是 | — | 否 | 键管理：DynamoDB 表达不了/代价过高 |
| `eval` | `s` | 0 | 否 | — | 否 | Lua 脚本：架构受限（需内嵌解释器） |
| `evalsha` | `s` | 0 | 否 | — | 否 | Lua 脚本：架构受限（需内嵌解释器） |
| `exec` | `sM` | 0 | 否 | — | 否 | 事务：架构受限（需原子多命令） |
...
| `publish` | `pltF` | 0 | 否 | — | 否 | 发布订阅：架构受限（需连接级订阅+跨连接 fan-out，无状态代理不适合） |
| `pubsub` | `pltR` | 0 | 否 | — | 否 | 发布订阅：架构受限（需连接级订阅+跨连接 fan-out，无状态代理不适合） |
| `punsubscribe` | `pslt` | 0 | 否 | — | 否 | 发布订阅：架构受限（需连接级订阅+跨连接 fan-out，无状态代理不适合） |
...
| `script` | `s` | 0 | 否 | — | 否 | Lua 脚本：架构受限（需内嵌解释器） |
| `shutdown` | `alt` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `slaveof` | `ast` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `sort` | `wm` | 1 | 是 | — | 否 | 键管理：DynamoDB 表达不了/代价过高 |
| `subscribe` | `pslt` | 0 | 否 | — | 否 | 发布订阅：架构受限（需连接级订阅+跨连接 fan-out，无状态代理不适合） |
| `sync` | `ars` | 0 | 是 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `unsubscribe` | `pslt` | 0 | 否 | — | 否 | 发布订阅：架构受限（需连接级订阅+跨连接 fan-out，无状态代理不适合） |
| `unwatch` | `sF` | 0 | 否 | — | 否 | 事务：架构受限（需原子多命令） |
| `wait` | `s` | 0 | 否 | — | 否 | 服务器/复制/管理：不适用于无状态代理 |
| `watch` | `sF` | 1 | 否 | — | 否 | 事务：架构受限（需原子多命令） |

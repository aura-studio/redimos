...
| ❌ **集合/哈希/zset 成员名上限 ~1023B**（写路径） | `SADD`/`HSET`/`ZADD` 存 name 超 1023B 的成员/字段回 `value exceeds backend limit`（v1.47.0 起写守卫按 `MaxMemberNameSize=1023` 判，恰 1024B 也回该确定性错误、不再穿透成 backend-error）；Redis 可存到 512MB | 成员/字段名是 DynamoDB **sort key** 的一部分（`0x01`前缀 + name），DynamoDB **sort key 硬上限 1024B**，故可存 name ≤1023B | 突破须改成员编码（把长 name 移出 sk、另存 value 并加二级索引查找），破坏 ZRANGEBYLEX/SSCAN 的 sk 有序扫描——大改。平台硬限，不修；**no-op 读**已全族对齐 Redis「不存在」：SISMEMBER/SREM/SMOVE（v1.45.0）+ HGET/HMGET/HDEL/HEXISTS/HSTRLEN/ZSCORE/ZRANK/ZREVRANK/ZREM（v1.47.0）回 nil/:0 |

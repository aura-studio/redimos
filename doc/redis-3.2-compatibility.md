...
| 分歧 | redimos 行为 | 原理（底层机制） | 成本（为何不修） |
|---|---|---|---|
| ❌ **一批命令代理拒绝** | `SORT`/`MOVE`/`RANDOMKEY`/`OBJECT`/`DUMP`/`RESTORE`/`MIGRATE`/`MULTI`/`EXEC`/`DISCARD`/`WATCH`/`SUBSCRIBE`/`PUBLISH`/`BLPOP`/`BRPOP`/`BRPOPLPUSH`/`EVAL`/`SCRIPT`/`SYNC`/`PSYNC`/`SLAVEOF`/`REPLCONF`/… 回一等专属错误（非「未知命令」） | **故意的功能边界**：无脚本/事务/阻塞/发布订阅/序列化/复制/集群——要么与「无状态代理 + 共享 DynamoDB 表」模型冲突（MULTI 事务、BLPOP 阻塞、SUBSCRIBE 长连、SYNC/SLAVEOF 复制），要么 DynamoDB 无对应原语（DUMP/RESTORE 序列化、SORT） | 逐个实现代价极高且部分与架构根本冲突（阻塞/事务/复制在共享表上是反模式）。有意不做，由 S 维度验证「拒得干净且连接存活」。见 `command-reference.md`。**注**：`RENAME`/`RENAMENX`/`KEYS`/`FLUSHDB`/`FLUSHALL`/`PERSIST` 现**已实现**（曾在早期版本被拒，此处不再列） |

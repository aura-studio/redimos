...
| ❌ **集合/哈希/zset 成员名上限 ~1023B**（写路径） | `SADD`/`HSET`/`ZADD` 存 name 超 1023B 的成员/字段回 `value exceeds backend limit`；Redis 可存到 512MB | 成员/字段名是 DynamoDB **sort key** 的一部分（`0x01`前缀 + name），DynamoDB **sort key 硬上限 1024B**，故可存 name ≤1023B | 突破须改成员编码（把长 name 移出 sk、另存 value 并加二级索引查找），破坏 ZRANGEBYLEX/SSCAN 的 sk 有序扫描——大改。平台硬限，不修；**no-op 读**（SISMEMBER/SREM/SMOVE 超限成员）已回 :0 对齐 Redis「不存在」（v1.45.0，§见版本历程） |

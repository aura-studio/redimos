...
| redimos **v1.45.0**(redimo 不动 v2.10.0) | **🔍 第四轮深评:18+7-lens 对抗验证工作流(finder→怀疑式 cull→真 `redis:3.2` oracle 逐条字节实证)找出并修 22 个可修·非平台受限分歧。** ①**整数解析 strtod/string2ll**——SCAN/HSCAN/SSCAN/ZSCAN 的 `COUNT`、BITCOUNT/BITPOS 的 start/end、SETBIT/GETBIT/BITFIELD 的 bit 偏移、BITFIELD 的 SET/INCRBY 值与类型位宽,全从 `strconv.Atoi`/`ParseInt`/`big.Int.SetString` 换成 `ParseInt`(string2ll):拒前导 `+`、拒前导零、拒 int64 溢出(旧代码静默回绕);②**LPUSHX/RPUSHX arity** −3→3(3.2 只收单值,多值→wrong-number-of-arguments,非多推);③**SET 重复标志松散循环**——重复 `NX`/`XX` 幂等接受、重复 `EX`/`PX` 末值胜、EX/PX 值只在循环后校验一次(`EX abc EX 10`→OK)、真正 EX↔PX 冲突才 syntax error;④**ZADD XX/类型检查序**——`XX`(及 `XX INCR`)在活异类型键回 WRONGTYPE(类型检查先于 XX-on-missing 短路,分数解析仍先于类型错以保 Redis 优先级);⑤**分数区间前导空白**——`ZRANGEBYSCORE " 1" 5` 按 strtod 跳前导空白视 1.0(而 ZADD 分数走 getDoubleFromObject 仍拒前导空白;尾随/全空白仍拒);⑥**错误文案/解析序**——GEO(负半径 `radius cannot be negative`、`COUNT must be > 0`、`need numeric radius`、`inf` 坐标按 C `%f` 非 Go `+Inf`、GEOADD 奇三元组提示、**GEORADIUS(BYMEMBER) 查表优先**:缺失键→`*0`、活异类型→WRONGTYPE 均先于任何坐标/半径解析)、ZUNIONSTORE/ZINTERSTORE `WEIGHTS` 非浮点回 `weight value is not a float`、ZRANGEBYLEX `LIMIT` 非整回 not-integer、CONFIG 未知子命令回子命令错(保留 CONFIG GET 探针桩)、PFDEBUG 拆「未知子命令」与「参数数错」两文案、BITPOS 非整 bit 参回 not-integer、BITCOUNT 参数数校验移到查表后(缺失键→:0、异类型→WRONGTYPE 优先于 syntax);⑦**类型检查先于解析**——INCRBYFLOAT 活异类型键先回 WRONGTYPE(readCurrentString 非创建式检查;HINCRBYFLOAT 经查证 3.2 本就解析先于查表,故不动)、HSCAN/SSCAN/ZSCAN 类型检查移到选项解析之前;⑧**超大成员 no-op**——SISMEMBER/SREM/SMOVE 对无法存储(sort key>1024B)的成员回 :0(不再让超限 SK 抵达后端回误导性 backend-error);⑨**幻影 `#meta` 字段/成员根治**——`store_hashes.HGetAll`/`store_sets.SMembers` 曾按**解码后字符串** `field==redimo.MetaSK` 二次过滤,而 fork 早已按 **sort-key 前缀 0x02** 排除真 meta 项→用户字面 `#meta` 字段/成员被误删(HGET 能取但 HGETALL/HKEYS/SMEMBERS 丢,数据丢失级);删掉冗余字符串过滤即根治;⑩**PFCOUNT/PFMERGE 估计器**——从 Ertl-2017(`hllSigma`/`hllTau`,Redis 4.0 才引入)换回 **Redis 3.2 遗留估计器**(调和均值 + LINEARCOUNTING + 偏差修正多项式,截断非四舍五入),对同寄存器各基数逐一与 `redis:3.2` 相等(旧版 N=1000 差 1);⑪**ZADD `NX XX` 无对**——对数校验先于 NX/XX 冲突→回 syntax error。测试:新增 `round4_fixes_test.go`+`round4b_fixes_test.go`(逐字节对 oracle)+`zsets_score_domain_test.go`(域拒单测)。**未修(记录,均对齐地板/平台)**:**INCRBYFLOAT 长双精度域**——3.2 用 80-bit long double(strtold,域达 ~1.19e4932),`1e309`/`1e400` 被接受;redimos float64(域 ~1.8e308)拒为 `not a valid float`——见 §4.1(需 big.Float + 复刻 long-double 舍入,成本高、纯 ultra-edge,接受不修);**glob `[^c]*` 匹配空串**——Redis 3.2 stringmatchlen 对空串**不自洽**:`[^c]*` 匹配 `""` 但 `[a-z]*`/`?*` 不匹配(负类+星特例,同 GETRANGE 属 C 控制流意外),无法干净复刻,记录不修——见 §4.5。redimos 全单元 + 全差分(docker)全绿,3 轮 |
| redimos **v1.46.0**(redimo 不动 v2.10.0) | **🔍 第五轮深评:11-lens 对抗验证工作流(含专门的「round-4 修复回归审计」lens)找出并修 8 个可修分歧——其中 2 个正是**前轮修复自身的完整性漏网**。** ①**十六进制小数浮点 `0x1.8`**——round4 的十六进制救援 `parseHexNoExp`/`parseHexNoExpStored` 只救**整数**(`0x10`→16)、遇 `.` 即 bail,而 glibc strtod 也接受**无 `p` 指数的十六进制小数**(`0x1.8`→1.5、`0x.8`→0.5);现两处只在已含 `p`/`P` 时 bail,否则补 `p0` 再解析——覆盖每个浮点入口(ZADD/ZINCRBY 分数、ZRANGEBYSCORE/ZCOUNT 界、INCRBYFLOAT/HINCRBYFLOAT 增量+已存值、GEO 坐标/半径、WEIGHTS);②**分数区间界溢出 `1e400`**——区间界**从不落库**,Redis zslParseRange 用 strtod 把超 float64 域的量级**饱和成 ±inf** 并接受(`ZRANGEBYSCORE z 1 1e400`→全返、`ZCOUNT z -inf 1e400`→全计),redimos 曾回 `min or max is not a float`;`parseScoreBound` 现在 parseScore 失败后用 `saturateOutOfRangeFloat`(strconv 的 ErrRange 饱和值)恢复——而 **ZADD 存分数 1e400 仍确定性拒**(§4.1 平台域,两侧一致);③**列表元素大小上限**——列表元素存 DynamoDB **value 属性**(位置在 `skN`),redimo 里 `element=listElement(items[0][vk])`,故上限应是 value 域(~390KB)而非成员名域(1KB);RPUSH/LPUSH/LPUSHX/RPUSHX/LSET/LINSERT 的 `guard.CheckWrite` 从「元素当成员名槽」改到「元素当值槽」→2KB 元素现可存(对齐 Redis);④**BITCOUNT 空串+非整界**——`SET E ""; BITCOUNT E 0 abc` 曾短路 :0(空串检查在解析前),现**先解析两个界**(非整→not-integer)再判空;⑤**BITCOUNT 双负钳位**——`BITCOUNT k -100 -200` 曾钳到 [0,0] 计 byte0=:4,现加 Redis 的**预钳守卫** `if start<0 && end<0 && start>end → :0`;⑥**AUTH 失败去认证**——AUTH 密码错时 `SetAuthed(false)`(Redis authCommand 置 `authenticated=0`):**已认证会话遇错误 AUTH 会被撤销**(后续命令回 NOAUTH),非仅拒本条;⑦**NOAUTH 门序**——`Router.Dispatch` 重构对齐 processCommand:**QUIT 特判**(先于查表/arity/auth,任意参数数回 +OK 并关连)→**查表+arity**(未知命令/参数数错先于 NOAUTH 上报)→**NOAUTH 门**;⑧**QUIT 额外参数**——`QUIT foo`→+OK 并关连(特判在 arity 之前),非 arity 错。测试:新增 `round5_fixes_test.go`(逐字节对 oracle)+ `connection_test.go` 三个 auth 序单测(用 requirepass 场景);auth 三项另用真 requirepass `redis:3.2`(:6391)+ requirepass 代理(:6381)raw-RESP 逐条实证。**未修(平台)**:SADD/HSET/ZADD 存**>1023B 成员/字段**——DynamoDB sort key 上限 1024B、成员=前缀1B+名,故 Redis 能存 2KB 成员而 redimos 不能(§4.1 SK 域,写路径平台限;no-op 读已回 :0)。redimos 全单元 + 全差分(docker)全绿。**趋势:round5 收成(8)少于 round4(22)、且 2/8 是「修复的完整性漏网」——长尾在收窄,但「回归审计」这一 lens 证明每轮修复仍会留缝,值得每轮自审** |
| redimos **v1.47.0**(redimo 不动 v2.10.0) | **🔍 第六轮深评:11-lens(含「round-5 修复回归审计」lens)找出并修 10 个分歧——回归审计又逮到 1 个前轮修复的漏网(HELLO)、超大成员 no-op 补齐哈希/zset 两族。** ①**INCRBYFLOAT/HINCRBYFLOAT 回复格式**——`formatRedisFloat` 曾用 Go 最短往返形式(`'f',-1`),微小量级打出 >17 位小数(`1e-20`→`0.00000000000000000001`);Redis 3.2 用 `ld2string(LD_STR_HUMAN)`=`%.17Lf`+去尾零(**17 位固定小数**,非最短):`1e-20`→`0`、`9e-18`→`0.00000000000000001`;改为 `%.17f`+trim(回复与落库值一致);②**HELLO 在 requirepass 下(round-5 漏网)**——round-5 的 NOAUTH 门序重构假设「查表命中的都是真 3.2 命令」,但 HELLO 是**唯一注册了却不存在于 3.2** 的命令(注册只为伪造 unknown-command 回复给 go-redis v9/redis-py 做 RESP2 回退信号);unauth+requirepass 下 HELLO 曾回 NOAUTH 吞掉了该信号,现豁免于门(真 3.2 lookupCommand 对 HELLO 返 NULL、unknown-command 先于 auth 检查);③**超大字段/成员 no-op 补齐哈希/zset**——round-4 的 `memberStorable` 短路只盖了 set 族;HGET/HMGET/HDEL/HEXISTS/HSTRLEN、ZSCORE/ZRANK/ZREVRANK/ZREM 对 >1023B 字段/成员曾把超限 SK 打到后端回 backend-error,现回 nil/:0(Redis「不存在」);④**成员写守卫差一修正**——`MaxNameSize=1024` 曾放行恰 1024B 成员→SK=1025B>DynamoDB 上限→误导性 backend-error;新增 `guard.MaxMemberNameSize=1023`(键名仍 1024,分区键预算充足),恰 1024B 成员现回干净确定性的 `value exceeds backend limit`(**与 Redis 能存仍属 §4.1 平台分歧,但错误可判**);⑤**类型检查先于大小守卫**——LPUSH/RPUSH(`ensureTypeExpiring` 提前)、APPEND/SETRANGE(`rmwString` 曾丢弃读回的 wrongType 标志、compute 回调里的大小守卫先跑,现 wrongType 先行返回 `meta.ErrWrongType`):活异类型键+超大值现回 WRONGTYPE 非 size 错;⑥**探针桩子命令错误对齐**——SLOWLOG(未知子命令或 GET/LEN/RESET 参数数错→`Unknown SLOWLOG subcommand or wrong # of args. Try GET, RESET, LEN.`,含 per-子命令 arity)、COMMAND(未知子命令/COUNT 带多余参→`Unknown subcommand or wrong number of arguments.`;bare/COUNT/INFO/GETKEYS 保留良性桩)、CLIENT(未知子命令如 5.0+ 的 ID、GETNAME/SETNAME 参数数错→`Syntax error, try CLIENT (LIST | KILL | GETNAME | SETNAME | PAUSE | REPLY)`;SETNAME 名字含空格/控制字符→`Client names cannot contain spaces, newlines or special characters.`,字符集校验 `'!'..'~'`;LIST/KILL/PAUSE/REPLY 保留 +OK 桩)、CONFIG(per-子命令 arity 对齐 configCommand badarity:GET=3/SET=4/RESETSTAT·REWRITE=2,错→`Wrong number of arguments for CONFIG <sub>` 原样回显子命令)。测试:新增 `round6_fixes_test.go`(逐字节对 oracle)+ `TestHelloUnauthGetsUnknownCommand` 单测 + guard 边界测试改 `MaxMemberNameSize` + SLOWLOG 单测更新。全单元 + 全差分(docker)全绿。**趋势:round6 收成(10)与 round5(8)持平,其中回归审计 1 个、「同根因跨族补齐」4 个(哈希/zset no-op)、探针桩 4 个——功能面新 bug 已断流,产出集中在「前轮修复的完整半径」与「有意桩的错误路径」** |
| redimos **v1.48.0**(redimo 不动 v2.10.0) | **🔍 第七轮深评:11-lens(含「round-6 修复回归审计」lens)修 12 个分歧,并**首次补齐一个真功能缺口(GEO STORE)**。** ①**🌟 GEORADIUS/GEORADIUSBYMEMBER STORE/STOREDIST 实现**——3.2 本就支持(此前 redimos 回 syntax error 属功能缺口):STORE 把命中成员按 geohash 分值写目标 zset、STOREDIST 写距离(按查询单位),回写入数;空结果删目标回 :0;STORE 与 WITHCOORD/WITHDIST/WITHHASH 互斥→`STORE option ... not compatible ...`;`GEORADIUS_RO`/`GEORADIUSBYMEMBER_RO` 禁 STORE→syntax error;逐字节对 oracle 验(STORE 分值精确;STOREDIST 分值差 17 位=float64-vs-long-double 精度地板 §4.1)。②**LINSERT/LSET no-op 先于大小守卫**——LINSERT 先扫 pivot(缺失回 :-1)、LSET 先判索引域(越界回 index out of range),再判值大小;超大值+缺失 pivot/越界索引现回 :-1/index-error 非 size 错。③**BITFIELD 类型检查先于大小守卫**——把 `CheckValueSize` 移进 rmwString 回调(在 WRONGTYPE 检查之后):活异类型键+超上限偏移回 WRONGTYPE。④**BITFIELD 偏移界改为「仅偏移<2^32」**(非 offset+width)——`GET u8 4294967295`/`#`形式在 2^32 前最后 width 位现有效(读越界回 0),对齐 getBitOffsetFromArgument(超上限写仍被值大小守卫拦=平台)。⑤**ZUNIONSTORE/ZINTERSTORE 源类型检查先于 WEIGHTS/AGGREGATE 解析**——异类型源+坏 WEIGHTS 回 WRONGTYPE 非 weight-not-a-float。⑥**GEO 成员读 no-op 补齐**(round-6 跨族补齐的延伸)——GEODIST/GEOPOS/GEOHASH/GEORADIUSBYMEMBER 对 >1023B 成员经 `memberPos` 回 nil/decode-fail(曾 backend-error)。⑦**ZADD NX/XX/CH flag 路径超大成员**(v1.47④ 漏网,只修了 fast/incr 路径)——把 `guard.CheckWrite` 提到 ZScore 循环前:超大成员回确定性 `value exceeds backend limit`(而非 backend-error;与 Redis 能存仍属 §4.1 member-SK 平台分歧)。⑧**CLIENT LIST/KILL/PAUSE/REPLY arity 门**(round-6 桩修 ⑥ 漏网)——按子命令 argc 门控:LIST==2/PAUSE==3(超时须整数,否则 timeout-not-int)/REPLY==3(on/off/skip,否则 syntax error)/KILL>=3;错则回 CLIENT 语法错(有效 LIST 仍 +OK 桩=§4.5)。⑨**未知命令回显 CRLF 净化**——`ErrUnknownCommand` 把命令名里的 `\r`/`\n` 映射为空格(对齐 Redis sdsmapchars),防注入客户端字节破坏 RESP 单行错误帧。测试:新增 `round7_fixes_test.go`(逐字节对 oracle,含 GEO STORE 全套)。**未修(平台,§4.1)**:超大**键名**(>~2046B,超 DynamoDB 分区键上限)的 EXISTS/TYPE/DEL/TTL 等回 backend-error 而非 absent——与成员-SK 同类平台限,但键>2046B 极病态、涉 8 个 handler,记录不修。**趋势:round7 收成(12)略高于 round6(10),因含 GEO STORE 这个真功能补齐 + 回归审计又逮 3 个前轮漏网(ZADD flag/CLIENT arity/LINSERT-LSET)——回归审计 lens 连续第 3 轮是最高产角** |
| redimos **v1.49.0**(redimo 不动 v2.10.0) | **🔍 第八轮深评:9-lens(含「round-7 修复回归审计」+ 新增「命令表完整性」lens)修 5 个可修分歧,并首次逮到「前轮修复方向本身过度」的过修回归。** ①**🌟 HSET arity −4→4(命令表完整性缺口,类似上轮 GEO STORE)**——Redis 3.2 `HSET` 精确 arity 4(单 field/value 对),多字段 `HSET k f1 v1 f2 v2` 是 4.0+ 扩展;redimos 曾注册 -4、静默多写并回计数(一个应报错的命令写入了数据),现精确 4→多字段一律 arity 错误(两个 finder 独立命中、均已 live 对拍)。②**INCRBYFLOAT/HINCRBYFLOAT 格式回归修正(round-6 的 `%.17f` 过度纠偏)**——round-6 为对齐 long-double `%.17Lf` 把格式从最短往返改 `%.17f`,但 **float64 的 `%.17f` 在第 17 位打出二进制噪声**(`0.1`→`0.10000000000000001`、`3.3`→`3.29999999999999982`、`100.001`→`100.00100000000000477`),而 Redis 的 long-double 误差落在 17 位小数之外、去尾零得干净短形式;改为**混合策略**——最短往返形式优先(复刻所有常用人类小数),**仅当最短形式小数位 >17(亚 1e-17 量级)才退回 `%.17f`+trim**(保 `1e-20`→`0`、`9e-18`→`0.00000000000000001` 的 17 位舍入);逐字节对 oracle 验常用小数(0.1/0.3/3.3/100.001/3.14159/-0.1/HINCRBYFLOAT)全对。残留仅 3 个真 float64-vs-long-double 地板(§4.1):`5e-18`(17/18 位小数舍入边界,long double→`0` vs float64→`0.00000000000000001`)、**累加漂移**(`0.1×3`=float64 `0.30000000000000004` vs long double `0.3`)、**>17 有效位噪声**(`1234.5678`→long double `...79999999999999997`)。③**DECRBY key −9223372036854775808 值相关判定 + 去自造文案**——redimos 曾对 decrement=MinInt64 无条件回**自造** `ERR decrement would overflow`(3.2 根本无此文案);Redis 3.2 把 decrement 取负(C 里 `-MinInt64` 回绕成 MinInt64)=`INCRBY MinInt64`,据**当前值**判(`0`→MinInt64、`5`→…803、`-1`→`increment or decrement would overflow`);删特判、直接 `incrBy(-n)`(Go 一元负回绕同 C),值判定与 INCRBY 精确一致;删 `resp.ErrDecrOverflow` 常量。④**CLIENT PAUSE 负超时(round-7 ③ 漏网)**——round-7 只校验整数性,负值被当合法超时回 +OK;现负毫秒回 `ERR timeout is negative`(对齐 getTimeoutFromObjectOrReply)。⑤**CLIENT KILL 空参(argc2)文案(round-7 arity 门副作用)**——round-7 把 kill 门设为 `argc>=3`,`CLIENT KILL`(argc2)落到 default 回了 CLIENT 用法长文案;3.2 clientCommand KILL 分支 argc==2 回通用 `ERR syntax error`,现 argc==2 短路回 `resp.ErrSyntax`。**⚖️ 验证驳回 2 个 finder 误报**(体现「幸存者须 oracle 实证」的价值):finder 另报 `OBJECT` arity 3→-2、`DEBUG` arity -1→-2,经真 `redis:3.2` 逐字节实证**均驳回**——OBJECT 的 2-token/4-token 在 3.2 objectCommand 本就回 arity 风格错误(`wrong number of arguments for 'object'`),redimos 现 arity=3 已逐字节对齐,改 -2 反会把它们送进 `errObjectUnsupported` 造**新**分歧;`DEBUG`(bare)在 3.2 回 `You must specify a subcommand for DEBUG...`(非 arity 错),改 arity 也不对齐,且 DEBUG 整体属 §4.5 拒绝命令。测试:新增 `round8_fixes_test.go`(逐字节对 oracle)+ 单测更新(HSET 单字段/多字段拒、DECRBY MinInt64 值判定)。全单元 + 全差分(docker,55.7s)全绿,零回归。**趋势:round8 收成(5)少于 round7(12);构成=1 个命令表完整性缺口(HSET arity)+ 1 个前轮修复的过度纠偏(round-6 INCRBYFLOAT 格式)+ 2 个 round-7 CLIENT 漏网 + 1 个自造文案。回归审计 lens 连续第 4 轮高产,且**首次逮到的不是「漏网(修少了)」而是「过修(修过头)」**——修复的方向性错误也在回归审计射程内** |
| redimos **v1.50.0**(redimo 不动 v2.10.0) | **🔍 第九轮深评:8-lens(**新增全命令表 arity 审计** lens——round-8 的 HSET −4→4 就是这条线,系统扫尽)修 4 个可修分歧。arity 审计对 173 条已注册命令逐条比对 3.2 命令表,**审计基本干净**(仅带出 WAIT 桩的参数校验缺失)——说明 round-8 的 HSET 修复后 arity 面已基本对齐。** ①**🌟 HINCRBYFLOAT inf/-inf 被误拒(3.2 真实语义不对称)**——Redis 3.2 的 hincrbyfloatCommand **缺少** incrbyfloatCommand 那条 `isnan/isinf` 结果守卫,故 `HINCRBYFLOAT k f inf`→存 `inf`、`-inf`→`-inf`、`inf+(-inf)`→存 `-nan`(哈希字段值本就以字符串存,可无损往返);而 INCRBYFLOAT 确实拒 inf(两端一致)。redimos 曾在 store_hashes 复用了 INCRBYFLOAT 的守卫误拒——现仅 HINCRBYFLOAT 去守卫 + `formatRedisFloat` 补非有限值(`+Inf`→`inf`/`-Inf`→`-inf`/`NaN`→`-nan`);`parseStoredFloat` 仍拒读回 nan(镜像 3.2 string2ld,故对 `-nan` 字段再 HINCRBYFLOAT 两端都错),INCRBYFLOAT 守卫保留。②**GETRANGE 双负越界(推翻 v1.44.0「不可修」结论)**——`GETRANGE "hello" -100 -200`→3.2 回 `""`(getrangeCommand 的前置守卫 `start<0 && end<0 && start>end` 在归一化前触发),redimos 曾 clamp-then-compare 回 `"h"`。v1.44.0 试修破坏 `0 <MinInt64>` 是因用了 clamp-based;**正确修法是照抄 Redis 的前置守卫**——它以 `start<0` 为条件,provably 不碰任何 `start>=0`(`0 -200`→h、`0 MinInt64`→h)或 `start<=end`(`MinInt64 MinInt64`→h、`MinInt64 -1`→hello)用例,逐字节对 oracle 8 例验证通过。③**WAIT 参数校验缺失**——stub 曾丢弃两参一律回 `:0`;3.2 waitCommand 先校验 numreplicas(非整→`value is not an integer or out of range`)再校验 timeout(非整→整数错、负→`timeout is negative`);现补前置校验(桩仍回 `:0`,与 round-8 CLIENT PAUSE 同模式)。④**SCAN 游标前导 `+`**——`SCAN +0` 曾回 `invalid cursor`;3.2 parseScanCursorOrReply 用 strtoul 容忍单个前导 `+`(`+0`==`0`);新增共享 `parseScanCursor` helper(容忍 `+`、仍拒前导空白/裸符号/负数)统一 SCAN/HSCAN/SSCAN/ZSCAN 四处(负游标回绕受不透明游标平台限,不复刻)。**deferred/未修**:ZINCRBY/ZADD INCR 的**结果**超 Number 域(如 `9e125+9e125`)时泄漏 `backend error` 而非 ZADD 那样的干净 `value is out of range`——值本身是 §4.1 域限(oracle 回 `1.8e+126`、proxy 存不下,修了也不 byte-match),纯 ultra-edge 错误形状,保持 §4.1 记录;COMMAND COUNT 桩回 `:0`(返 173≠3.2 的 176,§4.5 桩)。测试:新增 `round9_fixes_test.go`(HINCRBYFLOAT/WAIT/GETRANGE 逐字节)+ `scan_cursor_test.go`(parseScanCursor 单测)。全单元 + 全差分(docker,67.3s)全绿,零回归。**趋势:round9 收成(4)≈round8(5);arity 审计 lens 首轮即证 arity 面已基本干净(HSET 后无新 arity 缺口),产出转向 3.2 的**语义不对称**(HINCRBYFLOAT vs INCRBYFLOAT 对 inf)与**推翻旧「不可修」结论**(GETRANGE,证明「平台地板」标签也需定期用真源码复核)** |
| redimos **v1.51.0**(redimo 不动 v2.10.0) | **🔍 第十轮深评:6-lens,**新增 pika-migrate 迁移兼容维度**(3 lens:被拒写命令 / 命令形态回归 / Pika-vs-3.2 语义)+ round-9 回归 + 2 边角。修 3 个代码分歧 + 大批 stale 文档纠正与迁移限制文档化。** ①**ZINCRBY/ZADD INCR 结果超 Number 域(重估 round-9 的 defer,用户点名)**——超域时曾泄漏 `backend error, retry later`(把**永久性**域违规误报成**可重试**→带重试逻辑的客户端死循环),而 ZADD 直接路径 round-7 已回干净 `value is out of range`。修:`store.ZIncrBy` **复用它本就会做的那次 `ZSCORE`**(此前把分数丢弃只留 found),留下旧分数、预检 `old+delta` 域、超域在原生 ADD 前回 `ErrScoreOutOfRange`(**零额外 I/O**),旧分数保留无撕裂,与 ZADD 路径一致。②**🌟 空串浮点解析(pika 维度逼出的跨维度共享真 bug)**——Redis strtod 把空串 `""` 当 `0.0`;redimos 的 `parseScore` 早已补空串→0,但**共享的** `ParseFloat`(args.go)与存储层 `parseStoredFloat` 没跟上→`INCRBYFLOAT`/`HINCRBYFLOAT`/`GEOADD`/`GEORADIUS` 对空串报错。两处根修(仅 exact `""`→0;单空格与整数路径 `INCRBY`/`HINCRBY` 仍拒,与 string2ll 一致)。**pika 维度真影响**:全量 `SET k ""` 后 binlog 转发的 `INCRBYFLOAT k <delta>` 曾被静默丢→迁移计数漂移,现零丢失。③**SCAN `-0` 游标(round-9 漏网)**——round-9 的 `parseScanCursor` 只剥前导 `+`;3.2 strtoull 也收前导 `-`,`-0`/`-00` 回绕为 0=合法起始/终止游标,现接受;`-1`/`-N` 仍拒(映射非零不透明游标,平台限)。统一 SCAN/HSCAN/SSCAN/ZSCAN 四处。**⚖️ 记录不修**:TTL `SET EX`/`SETEX` 溢出(`SET EX 9223372036854775` 秒 3.2 保留、redimos 删)——是 Redis **C-UB 区**(实测 `SET EX 9300000000000000` oracle 竟 `EXISTS 1`,与 finder 阈值模型矛盾),且 round-2/v1.42.0 已**有意**决定 SET-EX 溢出=确定性即刻过期(§4.6),redimos 删是一致行为、不追随 C-UB;`COMMAND COUNT`=`:0`(命令集结构性不同、返 173≠176 似是而非,入 §4.5 有意不做)。**pika-migrate 兼容维度产出=以「真实迁移负载视角」复核**:多为**迁移限制文档**而非代码 bug(印证 redimos 对 pika 正常转发命令面覆盖良好)。`doc/pika-migrate.md` 加 §4.1「迁移完整性边界」:被拒写(`SORT STORE`/`MOVE`/`RESTORE`→静默丢)、超大值/成员/键名 guard 拒、批量 cap flag 须默认 0、`pksetexat`→`SETEX` TTL 漂移、多步浮点累加漂移、空串浮点(v1.51.0 已修零丢失)。**纠正多处 stale 文档**:§4.6 删「多字段 HSET 超集」+「DECRBY 判溢出拒」(round-8 均已对齐 3.2)、§4.5 拒绝清单删 `RENAME`/`RENAMENX`/`KEYS`/`FLUSHDB`/`FLUSHALL`(**⚠️此项被 round-11 判为误改并已还原**:它们是「`r.reg` 注册但仍拒绝」非「功能实现」,见 v1.51.1)、hashes.go `HINCRBYFLOAT` 注释更正。测试:新增 `round10_fixes_test.go`(空串浮点逐字节)+ `storage.TestScoreOutOfDomain` + `parseScanCursor` -0 用例 + `TestParseFloat` 空串。全单元 + 全差分(docker,116s)全绿,零回归。**趋势:pika 维度首轮——兼容审计的价值是逼出 1 个跨维度共享真 bug(空串浮点=既 3.2 对齐又迁移丢数)+ 系统刻画迁移真实边界(多为源侧用了 redimos 有意不支持的写);3 代码修 + 大量 stale 文档纠正,长尾继续收窄** |
| redimos **v1.51.1**(文档-only,redimo 不动 v2.10.0) | **🔍 第十一轮深评:6-lens(解析入口一致性审计[核心] + round-10 回归 + reply-shape/binary/keyspace/string-list 边角)。代码零改——parse-consistency 审计**干净**(空串→0/hex 无 p/下划线拒/前导空白/inf 取舍/string2ll 整数,各解析入口经 round-1~10 已一致),reply-shape/binary/string-list 三 lens 各 0 候选,是「已挖到底」的强收敛信号。** **唯一产出=回归审计逮到 round-10 我自己的文档误改**:round-10 用 `grep 'r.reg("RENAME"'` 看到注册就把 `RENAME`/`RENAMENX`/`KEYS`/`FLUSHDB`/`FLUSHALL` 当「功能已实现」从 §4.5 拒绝清单删除——但它们其实是「`r.reg` 注册了专用 handler、handler 无条件回 tailored 拒绝错误」(RENAME→`is not supported`(非原子整集合重写)、FLUSHDB/FLUSHALL→`disabled`(会擦全表)),**从用户视角仍是拒绝命令**(实测 proxy 逐条确认);只有 `PERSIST` 是真实现(清 TTL→`TTL -1`)。已还原 §4.5 拒绝清单 + 纠正 v1.51.0 行 + pika doc + memory。**教训:「已注册(r.reg)」≠「功能已实现」——判定命令是否可用必须看 handler 是否真做事、或直接对 proxy 实测,不能只 grep 注册**。平台受限复核(均 §4.1 已列):ZINCRBY 超域(round-10 已改干净错误)、SETBIT/SETRANGE >390KB guard。全差分 docker 全绿(无代码改)。**趋势:round11 零代码修——parse-consistency 这条系统线已挖尽,本轮价值=自纠一个文档回归 + 确认收敛。回归审计 lens 连续第 N 轮证明「每轮修/改都可能留缝」,包括**文档**的改** |
| redimos **v1.52.0**(redimo 不动 v2.10.0) | **🔍 用户点名核验「任何命令的空串是否兼容 / 多 DB 是否兼容」→ 62 例综合差分对 redis:3.2 实测,修 1 个空串游标漏网。** **空串全面兼容(61/62 逐字节一致 + 修 1)**:空串作**键**(GET/EXISTS/TYPE/HGET/…)、**值**(GET→`$0:`、APPEND/GETSET/SETRANGE)、**成员**(SADD `""`→:1、SMEMBERS→`[""]`、ZSCORE/ZRANK/SPOP,含幻影空成员根治)、**字段**(HGET/HKEYS/HEXISTS/HINCRBY)、**分数**(ZADD `""` m→score 0)、**pattern**(HSCAN MATCH `""`)——全部对齐;整数路径(INCR/INCRBY/EXPIRE/SETEX/SETBIT/GETRANGE/LSET/SET EX 的空串参数)两端都拒(string2ll 一致),仅浮点 INCRBYFLOAT/GEO 的空串=0(round-10 已修)。**唯一漏网并已修=SCAN/HSCAN/SSCAN/ZSCAN 空串游标 `SCAN ""`**——Redis `strtoull("")`→0、当游标 0 接受,redimos 曾回 `invalid cursor`;`parseScanCursor` 加 `len==0→(0,true)` 短路(与 round-10 的 `-0`/round-9 的 `+0` 同一 helper;单空格 ` ` 仍拒)。**多 DB 全面兼容(需 `-multi-db` 启动)**:`SELECT 0..15` 正常、**隔离真实**(DB1 键不见于 DB0、各 DB 保留自己的值——proxy 靠 pk `{db}:key` 编码天然隔离)、`SELECT 16`/`-1`/`""`/非整 越界错误逐字节一致(`invalid DB index`);注意 `DBSIZE`=:0 桩(§4.5)、`MOVE`/跨-DB 命令被拒(§4.5)、pika-migrate 只用 DB0。测试:`scan_cursor_test` 补 `""`/` ` 用例。全单元 + 全差分(docker,132.6s)全绿,零回归 |
| redimos **v1.52.1**(文档-only,redimo 不动 v2.10.0) | **🔍 第十二轮:补上「值 × 位置矩阵」维度**(对抗采样一直漏的正交轴,见 v1.52.0 空串漏网的教训)——**穷举**而非采样:整数边界值 {MinInt64, MaxInt64, MaxInt64+1(溢出), 0, -1} × 位置 {count(SPOP/SRANDMEMBER/SCAN COUNT), index(LINDEX/LSET/LRANGE/LTRIM/LREM), offset(SETRANGE/GETRANGE/SETBIT/GETBIT/BITCOUNT/BITPOS), TTL(EXPIRE/PEXPIRE/SETEX/SET EX/EXPIREAT), bit-value, increment(INCRBY/HINCRBY/ZADD score), DB(SELECT)} + 二进制键/成员(`\x00`/`\xff`/嵌入`\x00`) + inf/nan,共 76 格逐格对 redis:3.2 差分。**代码零改——0 个新可修缺口**:所有边界要么逐字节一致(如 SETBIT/GETBIT 超 2^32 偏移两端都回 `bit offset out of range`、SETRANGE/LINDEX 溢出偏移两端都 `not an integer`、EXPIRE 负/0 秒两端都即刻删),要么是**已文档化平台**(`SELECT MinInt64`→Redis `(int)INT64_MIN==0` 回 +OK 的 C-UB=§4.6、`ZADD/ZINCRBY inf`=§4.1 Number 域)。**唯一「意外」是正向的**:`SRANDMEMBER key <负巨值>` redimos 封顶(v1.33.0)、真 Redis 3.2 OOM 挂死(差分时把 oracle 拖 OOM 了)→§4.6 新增「redimos 更 robust」行 + 记基建注意。**结论:穷举边界矩阵证明 boundary 处理扎实、无采样漏网**(空串矩阵 v1.52.0 逮到 1 个 SCAN 漏,整数边界矩阵逮到 0)。基建:rawcmp 加 socket 超时版 `rawcmp_t.py`(防 oracle OOM 时挂死),Python 输出须 `-u` 免块缓冲丢结果 |
| redimos **v1.53.0**(redimo 不动 v2.10.0) | **🔍 第十三轮:全维度综合扫(11-lens:round-8~12 回归审计[star] + A 错误/arity、C 回复形状、E TTL/键空间、AC 原子性、M SCAN-glob、Q 协议、bitops、geo-hll、zset、string-hash-set)。** 修 1 个真缺口。**过程**:synth 分类阶段 API 连接中断挂了→从各 agent transcript **恢复 41 个原始候选**自行逐条对 oracle 实证(反而更可靠——抓到一个 finder 幻觉)。①**GEODIST 缺失键回复形状 `$-1`→`$0`**——Redis 3.2 `geodistCommand` 用 `lookupKeyReadOrReply(...emptybulk)`:**缺失键**回**空 bulk `$0`**(`""`),而**缺失成员**(键存在)才回 `$-1`(nil);redimos 曾把两者都当 `$-1`。改 `handleGeoDist` 按 `live` 分支(与 GEOPOS/GEOHASH 一致:`!live`→`BulkString("")`)。逐字节 0/12 diverge。**⚖️ 验证驳回 1 个 finder 误报**:GEO 距离单位大小写不敏感——finder 称 Redis 收 `KM`/`Mi`/`FT`(HIGH),但实测 oracle **两端都回同一 `unsupported unit` 错误**——Redis 3.2 GEO 单位**本就大小写敏感**,redimos 已匹配(纯 finder 对 Redis 行为的幻觉,被 oracle 验证当场抓掉)。**其余 ~39 候选**:regression lens 确认 round-8~12 修复**完好无回归**;平台/已文档(均 round-13 实测后归档):inline NUL/LF-only/TAB=**§4.4**(redcon;NUL/LF redimos 反而更 robust/更正确、TAB 更差)、**BITFIELD `i64` WRAP/SAT 溢出=§4.6**(redimos 回正确 2's-complement、**Redis 3.2 有符号溢出 UB 回错值** `-1`/`-2`、4.0+ 已修)、SETRANGE/SETBIT 超大 offset 撑大 >390KB=**§4.1**(redimos 干净拒、Redis 3.2 巨额分配崩连)、FLUSHDB/FLUSHALL 拒=§4.5;⚠️`GEOADD nan` 坐标经实测**两端都回 not-a-valid-float、非分歧**(又一 finder 幻觉))。测试:新增 `round13_fixes_test.go`。全单元 + 全差分(docker)全绿,零回归。**趋势:收成 round10(3)→11(0)→12(0)→13(1)**;regression-all 又是最高产角(10 候选,全"已覆盖"确认 + 逼出这 1 个 GEODIST 漏)。**工程教训**:synth 是单点故障,它挂了差点丢整轮 22 候选——靠 workflow 把每个 agent 输出落盘才从 transcript 恢复;**下轮应把 synth 做成可容错(或跳过 synth、直接对 finder 原始候选做 oracle 实证)** |
| redimos **v1.54.0**（测试维度扩充，redimo 不动 v2.10.0） | **➕ 新增「大小写敏感」维度 + 把 3 个横切矩阵固化为常规测试文件。** ①**大小写敏感维度(新)**：命令名 mixed-case + 所有选项关键字/子命令 mixed-case（SET `Ex`/`nX`、ZADD `Nx`/`cH`/`InCr`、`WiThScOrEs`/`LiMiT`/`WeIgHtS`/`AgGrEgAtE`、BITOP `aNd`、BITFIELD `GeT`/`oVeRfLoW`/`wRaP`、LINSERT `BeFoRe`、HSCAN `MaTcH`/`CoUnT`、GEORADIUS `WiThCoOrD`/`aSc`、CLIENT/CONFIG 子命令）逐条对 oracle 实测→**全部与 Redis 3.2 一致**：命令名/关键字**大小写不敏感**（strcasecmp），而 **GEO 距离单位 `km`/`mi`/`ft`/`m` 大小写敏感**（3.2 手写 lowercase-only 检查，`Km`/`KM` 两端都拒），redimos 精确匹配（**0 分歧，无需改码**）。②**把 round-8~13 期间用 rawcmp 反复跑的 3 个横切矩阵固化为常规 `test/integration/` 文件**（同 `differ` 双端框架，纳入标准 `go test` 差分套）：`casesensitivity_test.go`、`emptystring_test.go`（空串作键/值/成员/字段/分数/数值参数/游标逐角色）、`boundarymatrix_test.go`（整数边界 {MinInt64/MaxInt64/溢出/0/-1} × 位置 {count/index/offset/TTL/bit-value/increment/DB} + 二进制键成员 + inf；排除已文档化平台项 `SELECT MinInt64`=§4.6、`inf`=§4.1、及会 OOM 真 Redis 的 `SRANDMEMBER 负巨值`=§4.6）。**意义**：这 3 个是**正交于命令族/关注点的「横切值」轴**——采样式检测天然漏（`SCAN ""` 游标 11 轮才现），穷举矩阵才系统覆盖；现固化为回归测试，永久防倒退。见 §10.2。全单元 + 全差分（docker，integration 317s）全绿，零回归 |
...
|---|---|---|---|
| ❌ **值大小上限 ~390KB** | string/bitmap/HLL/**列表元素**值超过 ~390KB 即拒（`CheckValueSize`/`CheckValue`）；Redis 上限 512MB。**含 offset 撑大的情形**：`SETRANGE k <极大 offset> x` / `SETBIT k <极大 offset> 1`（结果串 >390KB）也走此拒回 `value exceeds backend limit`——而 **Redis 3.2 对同样输入会尝试巨额分配 →「Server closed the connection」/被 OOM killer 杀**（round-13 实测,故此类**勿发 oracle**),redimos 干净拒绝反而更 robust | DynamoDB **单项硬上限 400KB**（含 pk/sk/属性名开销后净值 ~390KB）；一个 string 值 / 一个列表元素就是一个 DynamoDB item 的 `val` 属性 | 突破须把大值**分片跨多 item** 存 + 读时重组，且 APPEND/SETRANGE/GETRANGE/BITCOUNT/BITPOS 等全部要改成跨片操作、丧失单项原子——大改。平台硬限，不修。（v1.46.0 已修一个**误判**：列表元素曾被当成员名按 1KB 卡，现按 value 域 390KB） |
| ❌ **集合/哈希/zset 成员名上限 ~1023B**（写路径） | `SADD`/`HSET`/`ZADD` 存 name 超 1023B 的成员/字段回 `value exceeds backend limit`（v1.47.0 起写守卫按 `MaxMemberNameSize=1023` 判，恰 1024B 也回该确定性错误、不再穿透成 backend-error；v1.48.0 补 ZADD NX/XX/CH flag 路径同样确定性拒）；Redis 可存到 512MB | 成员/字段名是 DynamoDB **sort key** 的一部分（`0x01`前缀 + name），DynamoDB **sort key 硬上限 1024B**，故可存 name ≤1023B | 突破须改成员编码（把长 name 移出 sk、另存 value 并加二级索引查找），破坏 ZRANGEBYLEX/SSCAN 的 sk 有序扫描——大改。平台硬限，不修；**no-op 读**已全族对齐 Redis「不存在」：SISMEMBER/SREM/SMOVE（v1.45.0）+ HGET/HMGET/HDEL/HEXISTS/HSTRLEN/ZSCORE/ZRANK/ZREVRANK/ZREM（v1.47.0）+ GEODIST/GEOPOS/GEOHASH/GEORADIUSBYMEMBER（v1.48.0）回 nil/:0/decode-fail |
| ⚙️ **键名上限 ~2046B**（超大键 read/no-op 回 backend-error 而非 absent） | 对 name 超 ~2046B 的键，`EXISTS`/`TYPE`/`DEL`/`TOUCH`/`TTL`/`PTTL` 等回 `ERR backend error, retry later`；Redis 视为不存在回 :0/none/:-2 | 键名进 DynamoDB **分区键**（`{db}:` 前缀 + name），分区键**硬上限 2048B**，故超 ~2046B 的键 GetItem/Query 直接 ValidationException | 与成员-SK 同类平台限。但 >2046B 的键极病态（真实客户端几乎不会用）、且要在 8+ 个 key-命令 handler 各加 `keyStorable` 短路，价值/成本不划算，**记录不修**（如需可仿 `memberStorable` 加 `keyStorable` 短路） |
| ❌ **浮点精度 float64 vs long double** | zset 分数 / INCRBYFLOAT 用 Go `float64`（IEEE754 双精，~15–17 位有效）；Redis 用 80-bit x87 long double（~18–19 位）→ ZSCORE 回读第 17 位后可不同、多次累加漂移。**INCRBYFLOAT/HINCRBYFLOAT 回复格式**自 v1.49.0 起用**混合策略**（最短往返形式优先，仅亚 1e-17 量级退回 `%.17f`+trim），常用人类小数（0.1/0.3/3.3/100.001/3.14159/-0.1）逐字节对齐 Redis；残留仅 3 个真地板：`5e-18`（17/18 位小数舍入边界）、累加漂移（`0.1×3`）、`>17` 有效位噪声（`1234.5678`） | Redis 分数是 long double；redimos 的分数**必须落 DynamoDB Number 属性**并进 `skN` 数值索引排序，Go 侧用 float64 承载 | 「换 `big.Float`」只治精度、**不治范围**（Number 域仍限 1e-130..1e125，见下），且 `big.Float` 无法进 DynamoDB 数值 LSI 排序。差分已用 `eqFloatClose` 容忍累加漂移；第 17 位差异现实无感知，**低价值**，不修 |
| ❌ **INCRBYFLOAT 长双精度域**（float64 vs long double 的**范围**，非精度） | 字符串 `INCRBYFLOAT`/`HINCRBYFLOAT` 的增量或已存值若有限但超 `float64` 域（`1e309`/`1e400`，介于 ~1.8e308 与 ~1.19e4932 之间）→ redimos 回 `ERR value is not a valid float`；Redis 3.2 **接受并返回该 ~1e400 十进制**（v1.45.0 起明确记录:此前 `parseFloatArg` 注释误称 Redis 也拒 1e400，实为把 double 的 `HUGE_VAL` 误当 long double 的 `HUGE_VALL`) | Redis 用 `strtold` 解析进 **80-bit x87 long double**（域达 ~1.19e4932），仅 `errno==ERANGE` 才拒;redimos 的 `ParseFloat`=`strconv.ParseFloat(s,64)`,任何超 float64 max 的量级都回 `(+Inf, ErrRange)`→拒,且 `IncrByFloat` 算术是 float64（`1e308+1e308=+Inf`） | 与 §4.1 精度行同源:彻底修需把增量+已存值都换 `big.Float`（或 long double 仿真）解析、算术、并**复刻 Redis long-double 的舍入与十进制格式化**——成本高;且真实边界是 ~1.19e4932（`1e4900` 收、`1e4933` 两端都拒），纯 ultra-edge。**接受不修** |
...
| ❌ **HLL 始终 dense 编码** | HyperLogLog 内部 blob 始终 dense；`GET` 一个 HLL 键拿到的原始字节、`PFDEBUG ENCODING` 恒回 `dense`，与 Redis（**从 sparse 起、超阈值才转 dense**）不同 | 寄存器（p=14→16384×6bit、MurmurHash64A）与 Redis 3.2 hll.c **逐位一致**，且估计器用 **Redis 3.2 遗留算法**（调和均值 + LINEARCOUNTING + 偏差修正多项式,截断非四舍五入——v1.45.0 从错用的 Ertl-2017 `hllSigma`/`hllTau`(Redis 4.0 才引入)换回,现 `PFCOUNT` 对同寄存器逐基数与 `redis:3.2` 相等),故 `PFCOUNT` 估值完全相同；只是省掉了 sparse 表示 | `PFCOUNT`（唯一有语义的输出）已精确等价；复刻 sparse→dense 转换只影响不透明 blob 字节与 `PFDEBUG`，无实际价值，不修 |
...
| ❌ **inline 命令转义/引号/分隔/终止符处理** | inline（非 RESP 数组）命令里 redcon 的 tokenizer 与 Redis 3.2 的 `sdssplitargs`/`processInlineBuffer` 多处有别（round-13 逐条实测）：①`\xNN` 转义、`\n\r\t\a\b\f\v` 处理、**未闭合引号**错误文本；②**嵌入 NUL**（`SET k\x00v 1`）：redimos 执行并回 `+OK`，**Redis 3.2 的 `strchr` 在 NUL 处停→无回复/挂住**（redimos 更 robust）；③**LF-only 终止符**（`PING\nPING\n`，无 CR）：redimos 正确逐行解析回两个 `+PONG`，**Redis 3.2 `processInlineBuffer` off-by-one→回 `+PONG` 后把 `ING` 当命令回 `unknown command 'ING'`**（redimos 更正确）；④**TAB 分隔**（`ECHO\thi`）：Redis `sdssplitargs` 按任意空白（含 `\t`）分词→`ECHO` + `hi`，redcon 只按空格分→整体一个 token→`unknown command 'ECHO\thi'`（此处 redimos 更差） | redimos 的 inline 解析继承第三方 wire 库 redcon，其 inline tokenizer 非 Redis 的 sdssplitargs/processInlineBuffer 逐字节移植（②③ 恰好 redimos 更对/更 robust，④ 更差） | 属 §4.4 同源（redcon 继承）；inline + 这些转义/NUL/LF/TAB 都是极罕见路径（真实客户端几乎都发 RESP 数组），修需 fork/换 redcon，不修 |

### 4.5 错误文本风格 / 未实现命令

| 分歧 | redimos 行为 | 原理（底层机制） | 成本（为何不修） |
|---|---|---|---|
| ❌ **一批命令代理拒绝** | `SORT`/`RENAME`/`RENAMENX`/`MOVE`/`KEYS`/`FLUSHDB`/`FLUSHALL`/`RANDOMKEY`/`OBJECT`/`DUMP`/`RESTORE`/`MIGRATE`/`MULTI`/`EXEC`/`DISCARD`/`WATCH`/`SUBSCRIBE`/`PUBLISH`/`BLPOP`/`BRPOP`/`BRPOPLPUSH`/`EVAL`/`SCRIPT`/`SYNC`/`PSYNC`/`SLAVEOF`/`REPLCONF`/… 回一等专属错误（非「未知命令」） | **故意的功能边界**：无脚本/事务/阻塞/发布订阅/序列化/复制/集群/全表擦除，或与「无状态代理 + 共享 DynamoDB 表」模型冲突。`RENAME`/`RENAMENX`（整集合成员在新 pk 下重写 + 原子搬 meta/TTL，非原子、与 `MOVE` 同类）、`KEYS`/`FLUSHDB`/`FLUSHALL`（全表 Scan / 会擦掉整张多租户共享表）等**以专用 handler 回 tailored 错误**（`r.reg` 注册但仍拒绝，非 `registerReject`），从用户视角=拒绝 | 逐个实现代价极高且部分与架构根本冲突（阻塞/事务/复制/全表操作在共享表上是反模式）。有意不做，由 S 维度验证「拒得干净且连接存活」。见 `command-reference.md`。**注**：`PERSIST` 是**真实现**（清 TTL 后 `TTL`→-1）；⚠️v1.51.0 曾误把「`r.reg` 已注册」当「功能已实现」而从本列删除 `RENAME`/`RENAMENX`/`KEYS`/`FLUSHDB`/`FLUSHALL`，v1.51.1（round-11 回归审计）已纠正——它们**仍是拒绝命令**，只是拒绝错误来自专用 handler |
...
| ❌ **`COMMAND COUNT` 恒返 `:0`（命令表内省，有意不对齐 176）** | `COMMAND COUNT` 回 `:0`（不返 redimos 真实注册数 ~173）；Redis 3.2 回 `:176` | redimos 的命令集**本就不等于** Redis 3.2 的 176 条命令表：①**少了** Redis 的安全/集群桩——`POST`/`HOST:`（Redis 用来检测并回警告、断连以防浏览器/HTTP 跨协议攻击的伪命令，非数据命令）、`RESTORE-ASKING`（Cluster 槽迁移内部命令）；②**多了**握手补充——`HELLO`（仅为对现代客户端回一句干净的 `unknown command` 做 RESP3 握手回退信号）、`QUIT`（真 Redis 在 processCommand 特判、不入命令表，redimos 显式注册）；另 ~20 条被 `registerReject` 的命令仍占注册槽 | **命令集结构性不同 → 没有任何整数能同时「对 redimos 诚实」又「等于 Redis 的 176」**：返真实 173 仍 ≠176 且似是而非（客户端可能当成「像 Redis 但差一点」，比一眼看穿的桩更坑）；硬编码 176 假装是 3.2 则不诚实且脆弱。故返 `:0` = 误导最小的「不建模」信号（同 DBSIZE）。**有意不做** |
| ❌ **glob `[^c]*` 匹配空串**（Redis C 控制流不自洽的 ultra-edge） | SSCAN/KEYS 等的 `MATCH [^c]*` 在 Redis 3.2 匹配空成员 `""`,但 `[a-z]*`/`?*` **不**匹配 `""`(负类+尾星有特例);redimos globMatch 一致地不匹配空串 | Redis stringmatchlen 里**特定 C 控制流的意外产物**(负类星特例),非可表达的规则;redimos 取「负类星与正类星一致」的干净行为 | 复刻须逐字节仿 Redis 的 C 控制流分支;极畸形输入、真实客户端不触发,记录不修。（**注:同类的 GETRANGE 双负越界 `-100 -200`→"" 已在 v1.50.0/round-9 修复**——Redis 的守卫 `start<0 && end<0 && start>end` 以 start<0 为条件,provably 不碰 `0 <MinInt64>`;v1.44.0 试修破坏该例是因用了 clamp-based 而非此前置守卫,现逐字节对 oracle 验证通过） |
...
| **`SRANDMEMBER key <负巨值>` 封顶而非 OOM（v1.33.0 P1）** | `SRANDMEMBER k -9223372036854775808`（负 count → 带重复返 \|count\| 个成员）在 redimos 被**封顶**、快速返回少量成员；**真 Redis 3.2 会尝试构造 9.2×10¹⁸ 元素的回复 → OOM/挂死**（round-12 实测差分时正是这条把 oracle 拖 OOM 了） | Redis 负-count SRANDMEMBER 按 \|count\| 分配结果槽，无上界；redimos v1.33.0 的 DoS 封顶拦住 | 有意**比 Redis 更 robust**——该输入在真 Redis 上是自杀式 DoS，redimos 封顶是更安全的行为，不追随其 OOM。⚠️**测试基建注意**：差分时勿把此类 `SRANDMEMBER 负巨 count` 发给 oracle（会 OOM 真 Redis） |
| **`BITFIELD i64` WRAP/SAT 溢出算术更正确（round-13 实测）** | `BITFIELD k SET i64 0 <MAX> OVERFLOW WRAP INCRBY i64 0 1` → redimos 回正确的 2's-complement 环绕 `INT64_MIN`；`MIN`−1→`MAX`；`OVERFLOW SAT` 下溢正确钳到 `MIN`。**Redis 3.2 因有符号 int64 溢出 UB 回错值**：`MAX`+1→`-1`、`MIN`−1→`-2`、SAT 下溢也回 `-2`（本应钳 `MIN`）。redis:4.0+/7 已修此 bug、与 redimos 一致 | Redis 3.2 的 bitfieldCommand 对 `i64` 走 C 有符号溢出（UB）；redimos 用 Go 明确的无符号环绕算 2's-complement | 有意**比 Redis 3.2 更正确**（且与 4.0+/7 对齐），不追随其 `i64` 溢出 UB。仅最宽有符号 `i64` 触发；`i1..i63`/`u1..u63` 两端一致 |
...
**再扩 3 个横切维度(round-8~13 深评期间用 rawcmp 反复跑的临时矩阵,v1.54.0 固化为常规 `test/integration/` 文件,同一 `differ` 双端框架,全部对真 Redis 3.2 实测)**:

| 维度 | 文件 | 检测内容 | 结果 |
|---|---|---|---|
| **大小写敏感** | `casesensitivity_test.go` | 命令名 mixed-case(`SeT`/`pInG`/…)+ 所有选项关键字/子命令 mixed-case(SET `Ex`/`nX`、ZADD `Nx`/`cH`/`InCr`、`WiThScOrEs`/`LiMiT`/`WeIgHtS`/`AgGrEgAtE`、BITOP `aNd`、BITFIELD `GeT`/`oVeRfLoW`/`wRaP`、LINSERT `BeFoRe`、HSCAN `MaTcH`/`CoUnT`、GEORADIUS `WiThCoOrD`/`aSc`、CLIENT/CONFIG 子命令)是否与 3.2 一致 | ✅ 逐字节一致——命令名/关键字**大小写不敏感**(strcasecmp);**GEO 距离单位 `km`/`mi`/`ft`/`m` 大小写敏感**(3.2 手写 lowercase-only 检查,`Km`/`KM` 两端都拒),redimos 精确匹配 |
| **空串横切** | `emptystring_test.go` | 空串 `""` 作**键/值/成员/字段/分数/数值参数/游标**逐角色:键(GET/EXISTS/TYPE/EXPIRE/DEL)、值(`$0` 往返/STRLEN/APPEND/GETSET/SETRANGE)、成员(SADD/SISMEMBER/SMEMBERS/ZADD/ZSCORE,含幻影空成员根治)、字段(HSET/HGET/HKEYS)、分数(`ZADD "" m`→0)、数值参数(浮点 `""`→0、整数路径仍拒、单空格拒)、游标(`SCAN ""`==游标 0) | ✅ 逐字节一致(round-8~12 逼出并修的 SCAN 空串游标、空串浮点在此锁定) |
| **值 × 位置矩阵** | `boundarymatrix_test.go` | 整数边界 {MinInt64, MaxInt64, MaxInt64+1(溢出), 0, -1} × 位置 {count(SPOP/SRANDMEMBER/SCAN COUNT)、index(LINDEX/LSET/LRANGE/LTRIM/LREM)、offset(GETRANGE/SETRANGE/SETBIT/GETBIT/BITCOUNT/BITPOS)、TTL(EXPIRE 族)、bit-value、increment(INCRBY/HINCRBY/ZADD)、DB(SELECT)} + 二进制键/成员(`\x00`/`\xff`/嵌入)+ inf/nan | ✅ 逐字节一致(**穷举**而非采样;已排除文档化平台项 `SELECT MinInt64`=§4.6、`ZADD/ZINCRBY inf`=§4.1,及会 OOM 真 Redis 的 `SRANDMEMBER 负巨值`=§4.6) |

> **这 3 个维度补的是「正交轴」**:A–P 按**命令族/关注点**切,而「大小写」「空串」「整数边界值」是**横切多个命令位置**的值——采样式检测天然漏(如 `SCAN ""` 游标 11 轮才被发现),须**穷举矩阵**覆盖。见 §2 v1.52.0/v1.52.1/v1.54.0。

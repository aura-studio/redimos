...
| ❌ **Redis C 控制流不自洽的 ultra-edge**（差分长尾，无法干净复刻） | 两处 Redis 3.2 自身 C 代码对**畸形/极值输入不自洽**、redimos 取「合理但简单」行为:①**glob `[^c]*` 匹配空串**——SSCAN/KEYS 等的 `MATCH [^c]*` 在 Redis 3.2 匹配空成员 `""`,但 `[a-z]*`/`?*` **不**匹配 `""`(负类+尾星有特例);redimos globMatch 一致地不匹配空串;②**GETRANGE 双负越界**——`GETRANGE "hello" -100 -200`→Redis `""` 但 redimos `"h"`,而 `0 -200`→Redis `"h"`、`0 <MinInt64>`→`"h"`(同样钳到 [0,0] 却结果不同,依赖 pre-clamp start 符号) | 两者都是 Redis stringmatchlen / getrangeCommand 里**特定 C 控制流的意外产物**(负类星特例、pre-clamp 符号泄漏),非可表达的规则;redimos 分别取「负类星与正类星一致」「clamp-then-compare 一致」的干净行为 | 复刻须逐字节仿 Redis 的 C 控制流分支;**试修 GETRANGE 曾被差分逮到破坏已测的 `0 MinInt64` 例**(见 v1.44.0 note)。极畸形输入、真实客户端不触发,记录不修 |

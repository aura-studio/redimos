...
func TestErrWrongType_CarriesRedisText(t *testing.T) {
	// The storage sentinel must carry the exact Redis WRONGTYPE text so the meta
	// layer and command handlers can reproduce Pika's reply byte-for-byte. On the v1
	// line redimo v1.6.1 has no ErrWrongType to mirror (rv1 does no type handling),
	// so the sentinel text is asserted directly against the fixed Redis wording.
	const wantWrongType = "WRONGTYPE Operation against a key holding the wrong kind of value"
	if ErrWrongType.Error() != wantWrongType {
		t.Fatalf("storage.ErrWrongType = %q, want %q", ErrWrongType.Error(), wantWrongType)
	}

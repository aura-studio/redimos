...
func (s *redimoStore) LPush(ctx context.Context, pk string, elements [][]byte) (int, error) {
	// v1 line: redimo v1.6.1's list impl is NOT binary-tolerant — lPush does an
	// unchecked e.(StringValue) type assertion (lists.go:178) and PANICS on a
	// BytesValue. So list elements MUST be wrapped as StringValue. A Go string holds
	// arbitrary bytes, so string(e) round-trips the exact bytes; the read path
	// (LPop/RPop/lAll/LRange) already decodes via ReturnValue.String(), so bytes are
	// preserved end-to-end. Every element is pushed, so the net cnt delta is len().
	if len(elements) == 0 {
		return 0, nil
	}
...
func (s *redimoStore) RPush(ctx context.Context, pk string, elements [][]byte) (int, error) {
	// StringValue as for LPush (rv1 lists are StringValue-only; see LPush).
	if len(elements) == 0 {
		return 0, nil
	}

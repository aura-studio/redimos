...
func (s *redimoStore) zAscending(ctx context.Context, pk string) ([]ZMember, error) {
	// v1 line: rv1 has no ZMembersOrdered. rv1 ZRANGEBYSCORE(key,-inf,+inf,0,0)
	// returns the FULL member->score map (offset 0, count 0 = no limit), which we
	// convert to []ZMember and re-sort in-proxy with SortZMembers to rebuild the
	// exact ascending (score, then byte-lex member) order the rank/score reads
	// expect. The returned map loses order, so the SortZMembers re-sort below is
	// load-bearing on the v1 line, not just a tie-break correction.
	scores, err := s.client.ZRANGEBYSCORE(pk, math.Inf(-1), math.Inf(1), 0, 0)
	if err != nil {
		return nil, err
	}

	out := make([]ZMember, 0, len(scores))

// Package storage wraps redimo (v1 line: redimo v1.6.1) that maps Redis data
...
// to this package so the backend can be swapped or mocked, and so the rest of the
// proxy can be unit-tested against a fake Store without a live DynamoDB.
//
// v1 line: the Store INTERFACE is preserved verbatim (so the command/meta layers
// compile unchanged), but redimo v1.6.1 has NO #meta item, NO type/cnt/TTL, and NO
// WRONGTYPE. The meta primitives (EnsureType/LoadMeta/SetExpire/Persist/DeleteMeta/
// …) are therefore re-expressed over rv1's raw high-level methods: counts come from
// rv1's partition COUNT (HLEN), existence from rv1.EXISTS, deletion from rv1.DEL,
// and type/expiry checks collapse to no-ops (type/TTL commands are GATED at the
// command layer). See store_core.go for the full mapping and the accepted
// tradeoffs (no WRONGTYPE, no TTL, best-effort INCR without a compare-and-set).
//
// ctx note: the Store methods still accept a ctx so the proxy API is context-aware,
// but redimo v1.6.1 has NO WithContext — every rv1 method uses context.TODO()
// internally — so on the v1 line the ctx is NOT propagated into the DynamoDB call.
// A --command-timeout can still cancel at the command layer, but the in-flight
// backend call does not observe the ctx.
package storage
...
// ErrWrongType is the storage-seam sentinel for a type conflict. The meta layer
// maps it onto its own sentinel, which command handlers ultimately translate to the
// RESP reply "-WRONGTYPE Operation against a key holding the wrong kind of value".
//
// v1 line: redimo v1.6.1 tracks no type, so this sentinel is NEVER returned by the
// v1 store implementation (WRONGTYPE is unenforceable). It is retained so the meta
// layer and the errors.Is plumbing compile unchanged; the value is a fixed string,
// no longer mirroring any redimo export.

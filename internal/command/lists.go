...
func (r *Router) registerLists() {
	...
	// v1 line GATE: LINSERT is NOT registered → "ERR unknown command 'linsert'".
	// redimo v1.6.1 exposes no LINSERT primitive and no bulk-replace method to build
	// a BEFORE/AFTER pivot insert atomically. handleLInsert stays compiled but
	// unreachable. (BLPOP/BRPOP are already first-class rejections in rejected.go.)

...
func (r *Router) registerStrings() {
	...
	// v1 line GATE (NOT registered → "ERR unknown command '<name>'"): SETEX, PSETEX
	// need per-key TTL (redimo v1.6.1 has none); APPEND, STRLEN, SETRANGE, GETRANGE,
	// SUBSTR need sub-value byte access / a compare-and-set RMW that rv1 does not
	// provide. Plain SET NX/XX proxy fine; the SET EX/PX option forms are rejected in
	// handleSet's option parser. Handlers below (handleSetEX/handleAppend/...) stay
...
func (r *Router) handleGet(ctx context.Context, c *server.Conn, args [][]byte) {
	...
	// v1 line: redimo v1.6.1 has no type tag, so WRONGTYPE is unenforceable on GET
	// (LoadMeta returns an empty Type). The read simply returns the String value item
	// under the pk (sk=""); a key that only holds collection items has no sk="" value
	// item and GetString reports found=false → null bulk. (On the v2 line this
	// compared m.Type != TypeString and replied WRONGTYPE.)
...
func (r *Router) handleSet(ctx context.Context, c *server.Conn, args [][]byte) {
	...
	// v1 line: redimo v1.6.1 has no TTL storage, so the SET EX/PX/EXAT/PXAT option
	// forms cannot be honoured. Reject them explicitly rather than silently dropping
	// the TTL (plain SET and SET NX/XX still work). This mirrors the GATE of
	// SETEX/PSETEX; without it a SET k v EX 10 would falsely reply +OK with no expiry.
	if o.expSet {
		w.Error("ERR SET with EX/PX/EXAT/PXAT is not supported on this proxy (no TTL support)")
		return
	}
...
func (r *Router) handleMGet(ctx context.Context, c *server.Conn, args [][]byte) {
	...
	for i, key := range keys {
		...
		// v1 line: no type tag, so a key is fetchable as a String iff it is live
		// (found & not expired). GetString returns the sk="" value item if present,
		// else absent → null bulk. (On the v2 line this also required m.Type ==
		// TypeString.)

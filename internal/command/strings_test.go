...
func TestSetEXWritesExpiryInSeconds(t *testing.T) {
	t.Skip("v1 line: SETEX is gated on redimo v1.6.1 (no TTL)")
...
func TestSetPXTruncatesToSeconds(t *testing.T) {
	t.Skip("v1 line: SET EX/PX is gated + no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startStringServer(t, store, fixedNow(1000))
...
func TestSetClearsExistingTTL(t *testing.T) {
	t.Skip("v1 line: no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startStringServer(t, store, fixedNow(1000))
...
func TestSetEXInvalidExpire(t *testing.T) {
	t.Skip("v1 line: SETEX/PSETEX are gated on redimo v1.6.1 (no TTL)")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
...
func TestPSetEX(t *testing.T) {
	t.Skip("v1 line: PSETEX is gated on redimo v1.6.1 (no TTL)")
	store := newFakeStringStore()
	conn, r := startStringServer(t, store, fixedNow(1000))
...
func TestSetOverwritesAnyType(t *testing.T) {
	t.Skip("v1 line: redimo v1.6.1 has no type tag (no WRONGTYPE) and SETEX is gated; the type-overwrite + GETSET-WRONGTYPE assertions here are v2-only")
...
func TestStringArityErrors(t *testing.T) {
	...
	cases := map[string]string{
		...
		// v1 line: SETEX/PSETEX are GATED (unregistered → "unknown command"), so
		// their arity is not checked here; those cases were removed.
	}
...
func TestMSetClearsTTL(t *testing.T) {
	t.Skip("v1 line: no TTL on redimo v1.6.1 (SET/MSET cannot clear a TTL that cannot exist)")
	store := newFakeStringStore()
	conn, r := startStringServer(t, store, fixedNow(1000))
...
func TestAppendCreatesKeyWhenAbsent(t *testing.T) {
	t.Skip("v1 line: APPEND is gated on redimo v1.6.1 (needs sub-value RMW/CAS)")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
...
func TestAppendPreservesTTL(t *testing.T) {
	t.Skip("v1 line: APPEND is gated + no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startStringServer(t, store, fixedNow(1000))
...
func TestAppendWrongType(t *testing.T) {
	t.Skip("v1 line: APPEND is gated + no WRONGTYPE on redimo v1.6.1")
	store := newFakeStringStore()
	store.metas["0:h"] = storage.Meta{Type: string(meta.TypeHash)}
	store.live["0:h"] = true
...
func TestStrlenPresent(t *testing.T) {
	t.Skip("v1 line: STRLEN is gated on redimo v1.6.1")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
	sendRead(t, conn, r, "SET k hello")
...
func TestStrlenExpiredIsZero(t *testing.T) {
	t.Skip("v1 line: STRLEN is gated + no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startStringServer(t, store, fixedNow(1000))
	sendRead(t, conn, r, "SET k hello")
	m := store.metas["0:k"]
...
func TestStrlenWrongType(t *testing.T) {
	t.Skip("v1 line: STRLEN is gated + no WRONGTYPE on redimo v1.6.1")
	store := newFakeStringStore()
	store.metas["0:h"] = storage.Meta{Type: string(meta.TypeHash)}
	store.live["0:h"] = true
...
func TestSetRangeExtendsExisting(t *testing.T) {
	t.Skip("v1 line: SETRANGE is gated on redimo v1.6.1 (needs sub-value write/CAS)")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
...
func TestSetRangeNonIntegerOffset(t *testing.T) {
	t.Skip("v1 line: SETRANGE is gated + no WRONGTYPE on redimo v1.6.1")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
	want := "-ERR value is not an integer or out of range"
...
func TestGetRangePositiveIndices(t *testing.T) {
	t.Skip("v1 line: GETRANGE is gated on redimo v1.6.1 (no sub-value read)")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
	sendRead(t, conn, r, "SET k Hello_World")
	// [0,4] -> "Hello".

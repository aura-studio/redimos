...
func (r *Router) registerHLL() {
	// v1 line GATE: PFADD/PFCOUNT/PFMERGE are NOT registered → "ERR unknown command
	// '<name>'". The HyperLogLog commands are implemented purely in the command layer
	// over the binary String value using a compare-and-set RMW (SetStringIfEquals),
	// which rv1 cannot provide; without it PFADD's lost-update safety is gone. PFDEBUG
	// (a debug-only command, not in Redis 3.2's client surface) is likewise dropped
	// since it is meaningless without the PF* family. Handlers below stay compiled but
	// unreachable.
}

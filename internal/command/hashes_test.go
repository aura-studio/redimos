...
func TestHDelLastFieldRemovesKey(t *testing.T) {
	...
	// v1 line: TYPE is gated; the empty-hash-deletes-key behavior is covered by
	// EXISTS (above) and HLEN (below).
...
func TestHStrlen(t *testing.T) {
	t.Skip("v1 line: HSTRLEN is gated on redimo v1.6.1 (no dedicated primitive)")
	conn, r := startStringServer(t, newFakeStringStore(), fixedNow(1000))
	sendRead(t, conn, r, "HSET h f hello")
...
func TestHashArityErrors(t *testing.T) {
	...
	cases := map[string]string{
		...
		// v1 line: HSTRLEN is GATED (unregistered → "unknown command"), so its arity
		// is not checked here; that case was removed. (HSCAN likewise gated.)
	}

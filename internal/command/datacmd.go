...
func encodePK(db int, key []byte) string {
	// v1 line: db0-only. The db argument is IGNORED and the prefix is always "0:".
	// SELECT n>0 is rejected at the connection layer (MultiDB is forced off), so in
	// practice db is already 0; forcing it here makes the encoding robustly single-db
	// even if a non-zero db ever reached a handler. The "0:" prefix is retained (not
	// dropped) so keys keep the same physical partition-key layout as before.
	return "0:" + string(key)
}
...
// ensureTypeExpiring is the type-ensure step for every collection / RMW write, with Redis'
// expire-if-needed semantics: a key that is only LOGICALLY expired (meta.exp <= now) and not
// yet reclaimed is treated as absent, so the write creates a fresh key of `expected` — even
// when the expired key is a DIFFERENT type (a bare EnsureType would reply WRONGTYPE) OR the
// SAME type (a bare EnsureType would succeed but leave it expired, swallowing the write).
//
// The atomic redimo primitive Meta.EnsureTypeExpiring does the expiry-aware type check in one
// conditional write (fast path: create-if-absent / add-to-live-same-type; fallback: take over
// an absent-or-expired key). When it reports it took over an expired key, we reclaim that
// key's now-hidden stale member items here (DeleteMembers) — the same reclaim contract SET NX
// uses after CreateTypeIfAbsent. A genuinely LIVE wrong-type key returns meta.ErrWrongType.
// The count is not touched here — callers apply the real delta via adjustCount afterwards
// (this ensure step always passes cntDelta 0).
func (r *Router) ensureTypeExpiring(ctx context.Context, pk string, expected meta.KeyType) error {
	_, tookOverExpired, err := r.Storage.Meta.EnsureTypeExpiring(ctx, pk, expected, 0, r.now())
	if err != nil {
		return err // meta.ErrWrongType (live wrong type) or a store error
	}
	if tookOverExpired {

...
func (r *Router) readCurrentString(ctx context.Context, pk string) (val []byte, found, wrongType bool, err error) {
	...
	// v1 line: redimo v1.6.1 has no type tag (LoadMeta returns an empty Type), so
	// WRONGTYPE is unenforceable — wrongType is never reported. A live key is treated
	// as a String; GetString returns its sk="" value item if present, else the key is
	// treated as an empty String. (On the v2 line this reported wrongType when
	// m.Type != TypeString; that path is what made INCRBYFLOAT wrongly reply WRONGTYPE
	// on the v1 line.)

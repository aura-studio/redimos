...
func TestSetRangeEmptyValueChecksType(t *testing.T) {
	t.Skip("v1 line: SETRANGE gated + no TYPE/WRONGTYPE on redimo v1.6.1")
...
func TestSetOverwriteReclaimsForeignMembers(t *testing.T) {
	t.Skip("v1 line: no TTL/meta foreign-member reclaim on redimo v1.6.1 (DEL reclaims synchronously; EXPIRE gated)")
...
func TestSetNXOverExpiredForeignTypeReclaims(t *testing.T) {
	t.Skip("v1 line: no TTL/expired-reclaim on redimo v1.6.1")
	store := newFakeStringStore()
	now := int64(1000)

...
func (r *Router) handleZAdd(ctx context.Context, c *server.Conn, args [][]byte) {
	...
	// score/member pairs => the argument count after the key must be even.
	if len(rest)%2 != 0 {
	...
	memberBytes := make([][]byte, 0, len(rest)/2)
	for i := 0; i < len(rest); i += 2 {
		...
		members = append(members, storage.ZMember{Member: string(rest[i+1]), Score: score})
		memberBytes = append(memberBytes, rest[i+1])
	}

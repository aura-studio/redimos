...
func TestDelExpiredKeyCountsZeroButCleansUp(t *testing.T) {
	t.Skip("v1 line: no TTL / expiry on redimo v1.6.1")
	store := newFakeStringStore()
	enq := &spyEnqueuer{}
	conn, r := startKeysServer(t, store, fixedNow(1000), enq)
...
func TestTypeString(t *testing.T) {
	t.Skip("v1 line: TYPE is gated on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)
...
func TestTypeExpiredReturnsNone(t *testing.T) {
	t.Skip("v1 line: TYPE is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
...
func TestKeysArityErrors(t *testing.T) {
	...
	cases := map[string]string{
		...
		// v1 line: TYPE is gated (unregistered), so its arity is not exercised here;
		// the two TYPE cases were dropped. Only the supported DEL/EXISTS arity is checked.
	}
...
func TestExpireSetsExpAndReturnsOne(t *testing.T) {
	t.Skip("v1 line: EXPIRE is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
...
func TestExpireAtSetsAbsoluteExp(t *testing.T) {
	t.Skip("v1 line: EXPIREAT is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
	sendRead(t, conn, r, "SET k v")
...
func TestPExpireTruncatesToSeconds(t *testing.T) {
	t.Skip("v1 line: PEXPIRE is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
	sendRead(t, conn, r, "SET k v")
...
func TestPExpireAtTruncatesToSeconds(t *testing.T) {
	t.Skip("v1 line: PEXPIREAT is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
	sendRead(t, conn, r, "SET k v")
...
func TestExpireNonIntegerReturnsError(t *testing.T) {
	t.Skip("v1 line: EXPIRE family is gated on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)
...
func TestTTLAbsentReturnsMinusTwo(t *testing.T) {
	t.Skip("v1 line: TTL/PTTL are gated / no TTL on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)
...
func TestTTLExpiredReturnsMinusTwo(t *testing.T) {
	t.Skip("v1 line: TTL is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
	sendRead(t, conn, r, "SET k v")
	m := store.metas["0:k"]
...
func TestTTLReturnsRemainingSeconds(t *testing.T) {
	t.Skip("v1 line: TTL/PTTL/EXPIRE are gated / no TTL on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)
	sendRead(t, conn, r, "SET k v")
...
func TestPersistRemovesExpiry(t *testing.T) {
	t.Skip("v1 line: PERSIST is gated / no TTL on redimo v1.6.1")
	store := newFakeStringStore()
	conn, r := startKeysServer(t, store, fixedNow(1000), nil)
	sendRead(t, conn, r, "SET k v")
	sendRead(t, conn, r, "EXPIRE k 60")
...
func TestTTLFamilyArityErrors(t *testing.T) {
	t.Skip("v1 line: EXPIRE/TTL/PERSIST family is gated on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)
	cases := map[string]string{
...
func TestKeysRejectedAsOpsOnly(t *testing.T) {
	t.Skip("v1 line: KEYS is gated on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)
...
func TestRenameFamilyRejected(t *testing.T) {
	t.Skip("v1 line: RENAME/RENAMENX are gated on redimo v1.6.1")
	conn, r := startKeysServer(t, newFakeStringStore(), fixedNow(1000), nil)

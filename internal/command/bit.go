...
func (r *Router) registerBit() {
	// v1 line GATE: the entire bit-operation family (SETBIT/GETBIT/BITCOUNT/BITPOS/
	// BITOP/BITFIELD) is NOT registered → "ERR unknown command '<name>'". redimo
	// v1.6.1 has no bit primitives, and the writing forms need a compare-and-set RMW
	// (SetStringIfEquals) that rv1 cannot provide safely. Handlers below stay compiled
	// but unreachable.
}
